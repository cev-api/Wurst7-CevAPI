	...
	implements NoDataSpecialModelRenderer
{
		...
		int lightCoords, int overlayCoords)
	{
		...
		for(CuboidModelElement element : model.elements())
		{
			...
			for(Map.Entry<Direction, CuboidFace> entry : element.faces()
				.entrySet())
			{
				...
				for(int i = 0; i < 4; i++)
				{
					...
					float u = CuboidFace.getU(uvs, face.rotation(), i);
					float v = CuboidFace.getV(uvs, face.rotation(), i);
	 ...
	 */
	private static CuboidFace.UVs defaultFaceUV(Vector3fc from, Vector3fc to,
		Direction facing)
	{
		return switch(facing)
		{
			case DOWN -> new CuboidFace.UVs(from.x(), from.z(), to.x(), to.z());
			case UP -> new CuboidFace.UVs(from.x(), from.z(), to.x(), to.z());
			case NORTH -> new CuboidFace.UVs(to.x(), to.z(), from.x(),
				from.z());
			case SOUTH -> new CuboidFace.UVs(from.x(), to.z(), to.x(),
				from.z());
			case WEST -> new CuboidFace.UVs(from.z(), to.z(), to.x(), from.z());
			case EAST -> new CuboidFace.UVs(from.z(), from.z(), to.x(), to.z());
		};
	}
}
