    ...
    CameraTransformViewBobbingListener, RenderListener
{
    ...
    private final SliderSetting lineWidth = new SliderSetting("Line width",
        2.0, 0.5, 10.0, 0.5, SliderSetting.ValueDisplay.DECIMAL);
        ...
        for(Entity entity : MC.level.entitiesForRendering())
        {
            if(entity.getType() == EntityType.ENDER_PEARL)
            {
                ...
                try
                {
                    ...
                    {
                        hasOwnPearl = true;
                        ownPearlSuppressUntilMs = System.currentTimeMillis()
                            + 4000; // suppress prediction up to 4s
                    }
    ...
    public void onRender(PoseStack matrixStack, float partialTicks)
    {
        ...
        if(MC.player != null)
        {
                ...
                if(isHoldingPearl(MC.player, hand))
                {
                    ...
                    // Suppress prediction if our pearl is in-flight or recently thrown
                    ...
                    if(pred != null && pred.landing() != null)
                    {
                        landingBoxes.add(createLandingBox(pred.landing(),
                            LANDING_BOX_RADIUS));
                        // draw with configurable width
                        RenderUtils.drawCurvedLine(matrixStack, pred.path(),
                            lineColor, false, lineWidth.getValue());
                    }
                }
            }
        }
        ...
        InteractionHand hand, float partialTicks)
    {
        ...
        // initial velocity based on look vector (approximate)
        Vec3 look = RotationUtils.getClientLookVec(partialTicks).normalize();
