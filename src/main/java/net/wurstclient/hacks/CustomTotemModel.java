 ...
 *
 * <p>
 * The geometry is the same <code>SimpleUnbakedGeometry</code> that
 * {@link BlockModel} would bake into regular quads; we just keep the raw
...
public final class CustomTotemModel
{
	...
	private CustomTotemModel(List<BlockElement> elements,
	...
	private static CustomTotemModel load()
	{
			...
			CustomTotemModel.class.getResourceAsStream(SKIN_MODEL_PATH))
		{
			...
			if(!(geometry instanceof SimpleUnbakedGeometry cuboid))
				return null;
	...
	private static Vector3fc[] computeExtents(List<BlockElement> elements)
	{
