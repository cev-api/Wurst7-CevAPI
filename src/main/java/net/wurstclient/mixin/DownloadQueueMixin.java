...
public abstract class DownloadQueueMixin implements AutoCloseable
{
	@Shadow
	@Final
	 ...
	 *
	 * <p>
	 * This mixin targets the <code>requests.forEach()</code> lambda in
	 * <code>runDownload(BatchConfig, Map)</code>.
	 *
		...
		Operation<Path> original)
	{
		...
		if(uuid == null) // shouldn't happen, but just in case
