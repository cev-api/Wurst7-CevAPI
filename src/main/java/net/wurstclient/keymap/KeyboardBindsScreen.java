...
public final class KeyboardBindsScreen extends Screen
{
		...
		int mouseY, float partialTicks)
	{
		...
		context.centeredText(font,
			"Click a key to edit its Wurst command, right click to clear.",
			width / 2, titleY + 12, CommonColors.LIGHT_GRAY);
		context.centeredText(font,
			"Vanilla Minecraft keybinds are shown on the keyboard too.",
		...
		int mouseX, int mouseY)
	{
		...
		if(hasWurst)
			fillColor = mix(fillColor, 0xFF3E2F1A, 0.18F);
		if(occupiedOnly)
			fillColor = mix(fillColor, 0xFF5E541B, 0.28F);
		if(conflict)
